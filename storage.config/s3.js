...
//====================================================
// List all podcast by a particular podcaster
//====================================================
async function ParticularPodcast(podcast_id) {
  // find the podcast in podcast category 
  const podcast = await podcastModel.findOne({ _id: podcast_id });

  if (podcast) {
    // use that podcast url to resign url 
    return {
      error: false,
    ...
    return {
      error: true,
      message: "No that podcast , or podcast id is incorrect.",
      data: {}
    }
  }
}
...
async function RegenerateUrlAllPodcast() {
  ...
  if (data != null) {
    ...
    for (let podcast of data) {
      ...
      const imgUrl = await getSignedUrl(S3, imgCommand, { expiresIn: 3600 * 2 });
      const audioUrl = await getSignedUrl(S3, audioCommand, { expiresIn: 3600 * 2 });
